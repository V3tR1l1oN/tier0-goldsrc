...
# Full tier0 sources — all tier0/*.cpp (Windows-only harnesses excluded on Linux).
SRCS     = $(filter-out $(SRCDIR)/testthread.cpp $(SRCDIR)/testthread2.cpp $(SRCDIR)/validator.cpp $(SRCDIR)/vprof.cpp,$(wildcard $(SRCDIR)/*.cpp))
