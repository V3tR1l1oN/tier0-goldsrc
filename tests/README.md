...
| `bench.cpp` | Бенчмарк внутренних хот-путей: стоимость `Plat_FloatTime`/`Plat_MSTime` (ns/call), стабильность часов (гэпы между соседними отсчётами frametime в чистом прогоне и под шумом-аллокатором), pacing тиков (`ThreadSleep(10/7)` — реальные интервалы пробуждения; legacy `Sleep` даёт 15.0/14.6 мс, точный сон — 10.02/7.00 мс), трип `Alloc`+`Free` по размером 4..65536, `Realloc` 64→2048, `GetSize` живых крупных блоков 8192/65536, thread create+join, кросс-поточный churn (4 потока alloc → main free); проверка `GetSize` (реальный размер ≥ заказанного) |
