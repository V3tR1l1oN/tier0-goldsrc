...
| `bench.cpp` | Бенчмарк внутренних хот-путей: стоимость `Plat_FloatTime`/`Plat_MSTime` (ns/call), трип `Alloc`+`Free` по размером 4..65536, `Realloc` 64→2048, `GetSize` живых крупных блоков 8192/65536, thread create+join, кросс-поточный churn (4 потока alloc → main free); проверка `GetSize` (реальный размер ≥ заказанного) |
